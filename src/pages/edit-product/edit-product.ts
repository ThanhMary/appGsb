...
export class EditProductPage {
  ...
  ionViewDidLoad() {
    this.categoryProvider.getAll()
      ...
      .catch(() => {
        this.toast.create({ message: 'Erreur en creant les categories.', duration: 3000, position: 'botton' }).present();
      });
  }
  ...
  save() {
    this.saveProduct()
      .then(() => {
        this.toast.create({ message: 'Produit save.', duration: 3000, position: 'botton' }).present();
        this.navCtrl.pop();
      })
      .catch(() => {
